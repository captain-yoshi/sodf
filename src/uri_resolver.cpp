...
Resolved resolve_resource_uri(const std::string& uri, const std::string& current_xml_dir, const char* env_roots)
{
  ...
  // sodf://... → search roots
  const char* roots_c = env_roots ? env_roots : std::getenv("SODF_DATABASE_PATH");
  if (!roots_c || std::string(roots_c).empty())
  {
    throw std::runtime_error("SODF_DATABASE_PATH not set (use ':' or ';' to separate multiple roots).");
  }
