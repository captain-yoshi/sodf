 ...
 *
 * Supported inputs:
 *  - sodf://vendor/object/.../file.ext  (searched under SODF_DATABASE_PATH roots)
 *  - file:///abs/path or absolute/relative filesystem paths
 *  - http(s)://...  (downloaded to cache if SODF_WITH_CURL enabled)
 *
 * Search roots:
 *  - Read from env var SODF_DATABASE_PATH (Unix ':' | Windows ';' separated)
 ...
 * @param env_roots       Optional override for SODF_DATABASE_PATH (tests)
